...
**TASK DETAILS**
1. First, you should summarize all user interactions from the video in sequential order and output them.
...
2. Then, you need to merge user interactions into unit actions according to the following rules:
-When a subsequent interaction depends on the result of a previous interaction that causes a page change, they cannot be merged into a unit action.
For example, Right-clicking a file and selecting "Properties" should be divided into two separate interactions, as the "Properties" button can only be located after right-clicking the file.
-If two consecutive interactions only need to follow the order of execution and do not require checking the result of the first interaction before proceeding, they can be merged into a unit action.
For example, in a keyword search, entering a keyword and clicking the "Search" button can be merged into a unit action because entering the keyword does not change the position or existence of the button.
"""
...
Each element in the list is a natural language description containing exactly one unit action and its purpose.
"""
